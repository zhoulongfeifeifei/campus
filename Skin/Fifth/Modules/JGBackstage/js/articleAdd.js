...
if(!Id) {
	...
	commonFn(GetArticle, function(t) {
        ...
        check(t.articleInfo.isTop, $("#IsTop"));
        check(t.articleInfo.hasTiming, $("#HasTime"));
		form.render();
...
function column(id) {
	...
	commonFn(GetListCatalogsTree, function(Data) {
		...
		$('#add').click(function() {
			var Id = IDs;
			var val=$("input[name='lm_name']").val();
			if(Id==''){
				layer.msg('请先选择栏目', {shift: 6,time: 1000,offset: '100px'})
			}else{
...
$('#Sava').click(function() {
	...
	var model = {};
	model.Title = $('input[name="artname"]').val();//得到文章标题
	model.IsNotify = $('input[name="isNotify"]').val();//得到是否通知的状态码 0 不通知 || 1通知
	model.SubTitle = $('input[name="artnametwo"]').val();//得到文章的副标题
	model.Descript = $('textarea[name="abstract"]').val();//得到简介的内容
    model.isTop = $('input[name="isTop"]').val();//得到是否置顶    默认0 布置顶 || 1置顶
    model.hasTime = $('input[name="hasTime"]').val();//得到是否开启定时  默认0  不开启 || 1开启
	// 文章上传
