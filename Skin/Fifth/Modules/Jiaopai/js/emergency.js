    ...
    function aa(current) {
        ...
        commonFn({ url: 'XAWebCommon/GetList_Notice', type: 'POST', data: data }, function (res) {
            ...
            $.each(info, function (i) {
                ...
                if (info[i].userId == getCookieByUserInfo().userid) {
                    cao = '<a class="text link" href="addEmergency.html?clint=' + info[i].device + '&id=' + info[i].id + '&sta=' + info[i].state + '">编辑</a>' +
                        '&nbsp;&nbsp;<a class="text green link publish" val="' + info[i].id + '" data_id="' + info[i].device + '">重新发布</a>' +
                        '&nbsp;&nbsp;<a class="text red link del" val="' + info[i].id + '">删除</a>'
                } else {
                    cao = '本人可编辑'
                }
                var intime = info[i].intime.replace('T', ' ').substr(0, 19);
                var publishDate = info[i].publishDate.replace('T', ' ').substr(0, 19);
                $('.tabl .layui-table tbody').append('<tr><td>' + info[i].title + '</td><td ><p class="text-shenglue" style="width: 300px;">' + info[i].detail +
                    '</p></td><td>' + intime + '</td><td>' + publishDate + '</td><td>' + st + t + '</td><td>' + info[i].signName + '</td><td>' +
                    info[i].name + '</td><td class="edit">' + cao + '</td></tr>')
            })
            ...
            $(".publish").click(function () {
                ...
                commonFn({ url: 'XAWebCommon/Update_Notice', type: 'POST', data: model }, function (res) {
                    ...
                    // } 
                    location.href = 'emergency.html' + window.location.search;
