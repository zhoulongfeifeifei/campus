...
commonFn(GetModelApprove, function(data) {
	...
	if(data.stepList.length>0) {
		$.each(data.stepList,function(i,n){
			if(i=="0"){
				htm=solveTime(n.inTime) + '&nbsp;' + n.userName + '&nbsp;' +getspstate(n.state)+"<br/>";
			}else{
				htm+=solveTime(n.inTime) + '&nbsp;' + n.userName + '&nbsp;' +getspstate(n.state)+"<br/>";
			}
		})
...
//状态操作
//确认
$("body").on("click", "a[data-id='f_yes']", function() {
	layer.confirm('你确定要确认该条抄送吗？', {
		icon: 3,
		title: false
	}, function(index) {
		layer.close(index);
		var chexiaoload = layer.load();
	 	var dele ={url:"Examine/PostExamineChaoSong",data:{
				ExamId: Id,
				State:2,
				schoolid:window.schoolid
			},type:"post"};
	  	commonFn(dele,function(data){
			...
			layer.msg("操作成功!", {
				...
				window.location.href = "ReceivedCc.html";
			});
...
//撤回
$("body").on("click", "a[data-id='f_no']", function() {
	layer.confirm('你确定要撤回该条抄送吗？', {
		icon: 3,
		title: false
	}, function(index) {
		layer.close(index);
		var chexiaoload = layer.load();
		var dele ={url:"Examine/RemoveChaoSong",data:{
				examId: Id,
				schoolid:window.schoolid
			},type:"post"};
	  	commonFn(dele,function(data){
	...
	layer.open({
		...
		yes: function(index, layor) {
			...
			if(UserID == olduserid){
				...
				var dele ={url:"Examine/ChangeExamineChaoSong",data:{
					ExamID: Id,
					UserId: UserID,
					OldUserId: olduserid,
					schoolid:window.schoolid
