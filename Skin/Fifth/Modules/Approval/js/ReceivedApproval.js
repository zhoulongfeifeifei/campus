...
function obtainList(current,sele,name) {
	...
	$('tbody').empty();
	var GetToMeExamineList ={url:"Examine/GetListExamineToMe",type:"post",data: {
			PageIndex: current,
			PageSize: 10,
			HadShenpi: sele,
			Title: name,
	...
	commonFn(GetToMeExamineList,function(data){
			...
			$.each(data.resultData, function(i, n) {
				...
				var row = '<tr  data-id="' + n.id + '" id="roleRow">' +
					...
					'<a href="ApprovalDetail.html" onclick="local(this)" data-id="xq" class="layui-btn layui-btn-mini">详情</a>' +
					'</td>' +
					'</tr>';
...
////获取状态
//var GetFromMeExamineList ={url:"Examine/GetListStatus",type:"get",data: {type:2,schoolid:window.schoolid}};
