...
form.verify({
	...
	phone: [/^13[0-9]{1}[0-9]{8}$|15[0-9]{1}[0-9]{8}$|18[0-9]{1}[0-9]{8}$|170[0-9]{8}$|14[0-9]{1}[0-9]{8}$/, '手机格式不正确'],
	...
	$.ajax({
		...
		success: function(data) {
			...
			for(var i = 0; i < volunteerNum; i++) {
				...
				zhiyuanHtml += '<div class="layui-form-item"><label class="layui-form-label">第' + r + '志愿：</label><br><div class="layui-input-inline"><select class="zyx" style="display:block;" name="" id="select' + (i + 1) + '" lay-filter="zhiyuan1" sort=' + (i + 1) + ' ><option value="0" sort=' + (i + 1) + '>请选择</option>';
...
//				$('input').attr('disabled','disabled');
				$('.zhuxiao input').each(function(data){
					$(this).attr('disabled','disabled');
				});
				$('#xingbie input').each(function(data){
					$(this).attr('disabled','disabled');
				})
				$('.zyx').each(function(k, v) {
